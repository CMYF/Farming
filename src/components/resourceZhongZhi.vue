    	...
    	<el-col  class="product-box">
            ...
            <div class="product-tab-box">
                <el-table ref="singleTable" border :data="tableData"  highlight-current-row style="width: 100%;text-align: center; " @cell-mouse-enter="showCode"  @current-change="danXuanChange" >
                    <el-table-column type="index" label="序号" width="150" style="width: 10%">
                    </el-table-column>
                    <el-table-column property="names" label="资源名称" style="width: 20%" className="zhongzhi-name-td">
                    ...
                    <el-table-column property="code" label="资源编号"  style="width: 10%">
                    </el-table-column>
                    <el-table-column property="usetime" label="开始使用时间"  style="width: 20%">
                    </el-table-column>
                    <el-table-column property="statusName" label="使用状态"  style="width: 10%">
                    	 <template scope="scope">
        ...
        <el-dialog class="dialog-boxzz" title="新增资源" :visible.sync="isShowPlanDailog" @close="closeDialogz">
		            <el-form :model="ymAddResource" :inline="true" :rules="rules"  ref="ymAddResource" class="plan-form" >
		                ...
		                <el-form-item label="资源类型" :label-width="formLabelWidth"  prop = "zyTypes">
		                    <el-select v-model="ymAddResource.zyTypes"  placeholder="请选择" @change="resourceTpye"  >
							    <el-option
...
export default {
    ...
    beforeMount() {
    	...
    	bus.$on('zhongZhiList', (test) => {
    		
    		test=this.ziYuanDate;
    		test.AllzyType = '2,3';
    		console.log(test)
			fetchTaskZiyuan(this.$store, test).then(() => {
    ...
    methods: {
    	...
    	showCode(row, column, cell, event){
            let dom = _j(cell);
    		 if (!dom.hasClass('zhongzhi-name-td')) {
                return;
           }
    	...
    	resourceTpye(value1){
    		...
    		if(value1){
    			let obj = {};
		      obj = this.typeName.find((item)=>{
		          return item.value === value1;
		      });
		     this.dzAllType = obj.el;
		     console.log("23232323")
		      console.log(this.ymAddResource.zyTypes)
...
.zhongzhi-name-active {
    color: #02bdad;
}

.zhongzhi-name-td:hover {
    color: #02bdad;
}
